...
/**
 * If ASLEEP_VOICE_CHANNEL_ID is configured and the member is currently
 * connected to any voice channel, move them into it.
 */
async function moveToAsleepVoiceChannel(member) {
  if (!ASLEEP_VOICE_CHANNEL_ID) return; // Feature not configured, skip.
  if (!member.voice.channel) return; // Not in a voice channel, nothing to move.
  if (member.voice.channelId === ASLEEP_VOICE_CHANNEL_ID) return; // Already there.

  await member.voice.setChannel(ASLEEP_VOICE_CHANNEL_ID).catch((err) =>
    console.error(`Failed to move ${member.id} to asleep voice channel:`, err)
  );
}
...
async function handleReaction(reaction, user, added) {
  ...
  if (added) {
    ...
    if (targetRole === 'asleep') {
      await moveToAsleepVoiceChannel(member);
    }
  } else {
