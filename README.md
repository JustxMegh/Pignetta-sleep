...
- Reacting ❌ → user gets "awake" (their "asleep" role is removed first,
  and that duration is logged). If `AWAKE_VOICE_CHANNEL_ID` is set and
  the user is currently in any voice channel, they're also moved into it.
...
If you're using `ASLEEP_VOICE_CHANNEL_ID` and/or `AWAKE_VOICE_CHANNEL_ID`
to auto-move users between voice channels, the bot's role also needs
**Move Members** permission, and needs to be able to see/connect to both
the voice channel the user is currently in and whichever channel it's
moving them to (permission overwrites on either channel can block this
even if the bot has the permission server-wide).
